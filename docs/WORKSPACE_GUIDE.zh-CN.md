...
每条已完成的用户或助手消息下方都有一组操作：

- **复制消息**：复制消息正文。
- **引用到输入框**：将消息作为引用插入当前输入区。
- **分支到新聊天**：从这条消息开始创建独立会话，保留它之前的上下文；原会话不会修改。打开新分支后，可以选择当前连接或其他模型继续。
- 用户消息还可以**编辑并重新发送**，助手消息还可以**重新生成回答**。

分支会出现在左侧会话列表中，标题通常带有“· 分支”。它是 ChatSwitch 的私有会话快照，不会写回原始 ChatGPT、Codex 或 Claude Code 文件。
