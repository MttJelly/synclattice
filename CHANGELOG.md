...
- OpenAI 官方 Responses 连接支持 Files API 原生文档上传；上传前明确确认隐私，完成、失败或中断后均请求删除临时文件。
- 增加统一附件文本提取模块，支持 TXT、Markdown、CSV、JSON、PDF、DOCX、XLSX 和 PPTX 作为模型上下文。
- XLSX 支持按工作簿顺序读取全部工作表，PPTX 支持按演示文稿顺序读取全部幻灯片。
- 会话支持生成并导出 Markdown、HTML、PDF 和 JSON，并增加统一文件写入测试。

### 优化

- PDF 改用内置 JavaScript 解析，不再依赖系统安装 `pdftotext.exe`。
- 普通中转连接继续使用本地文本提取，不会静默上传原始文档；OpenAI 官方原生上传与本地提取路径保持明确隔离。
- README 增加文件能力矩阵、隐私边界、语义 emoji 和更清晰的下载、功能、文档层级。

### 修复

- 修复 XLSX 只能读取第一个工作表、PPTX 只能读取第一张幻灯片的问题。
- 修复 Markdown 导出包含字面量换行转义、JSON 导出末尾写入字面量 `\\n` 的问题。
- 修复 PDF、Office 或模型请求失败时可能产生含糊错误或遗留临时上传文件的问题。

### 验证

- 72 项单元测试、完整隔离 Vue UI、会话分支、多窗口、线程操作、Project、模型设置、官方登录与线程性能测试通过。
- TXT、Markdown、CSV、JSON、PDF、DOCX、XLSX、PPTX 读取，以及 Markdown、HTML、PDF、JSON 生成矩阵通过。
