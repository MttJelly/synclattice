...
assert.equal(summary.isolatedApprovalModeEditable, true);
...
assert.deepEqual(summary.internalServerTypes, ["CodexServer", "CodexServer"]);
assert.deepEqual(summary.internalChildProcesses, [true, true]);
assert.deepEqual(summary.internalRuntimeKinds, ["chatswitch-bundled", "chatswitch-bundled"]);
assert.equal(summary.windows[0].providerName, "OpenAI 官方");
assert.equal(summary.windows[1].providerName, "Hexuan");
assert.ok(["low", "medium", "high", "xhigh", "max", "ultra"].includes(summary.windows[0].selectedEffort));
assert.equal(summary.windows[1].selectedEffort, "low");
...
assert.ok(summary.windows.every((window) => !window.approvalModeDisabled));
assert.ok(summary.windows.every((window) => window.approvalModeLabel === "请求批准"));
